...
window.fbAsyncInit = function() {
	FB.init({
		appId : '472323286188757', //change the appId to your appId
		status : true,
		cookie : true,
		xfbml : true,
		oauth : true
	...
	FB.Event.subscribe('auth.statusChange', updateButton);
		...
		e.async = true;
		e.src = document.location.protocol + '../static/js/all.js';
...
function fqlQuery() {
	...
	FB.api('/me', function(response) {
		...
		query.wait(function(rows) {
			document.getElementById('debug').innerHTML = 'FQL Information: ' + "<br />" + 'Your name: ' + rows[0].name + "<br />" + 'Your Sex: ' + (rows[0].sex != undefined ? rows[0].sex : "") + "<br />" + 'Your Profile: ' + "<a href='" + rows[0].profile_url + "'>" + rows[0].profile_url + "</a>" + "<br />" + '<img src="' + rows[0].pic_small + '" alt="" />' + "<br />";
		});
	});
}
...
function sendRequestToRecipients() {
	...
	FB.getLoginStatus(function(response) {
		if (response.status === 'connected') {
			$(".alert").alert('close');
			concole.log("achu is a pottan");
			var uid = response.authResponse.userID;
			var accessToken = response.authResponse.accessToken;
		} else {
			concole.log("achu is a pottan2");
			$(".alert").alert();
			 $("#add_crew").remove();
		}
	}); 